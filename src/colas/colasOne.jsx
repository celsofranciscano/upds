...
function ColasOne() {
  ...
  const [pila2, setPila2] = useState(new Pila());
  ...
  const popPila = () => {
    if (!pila.estaVacia()) {
      const elemento = pila.desapilar();
      const nuevaPila2 = new Pila([...pila2.elementos, elemento]); // Crear una nueva pila con el elemento desapilado agregado
      setPila2(nuevaPila2);
    }
  };
  ...
  return (
    <div className="grid md:grid-cols-2 gap-2  bg-white rounded-md ">
      ...
      <section className="border grid grid-cols-2 border-zinc-200 rounded-md shadow-md p-4">
        <h1 className="text-xl col-span-2">PILA CON DATOS DE SALIDA INVERTIDO </h1>
        {/* Interfaz de Pila 1 */}
        ...
        {/* Interfaz de Pila 2 */}
        {/* <div className="border-l-4 border-b-4 border-r-4 min-h-80 border-blue-500 w-24 p-1 flex flex-col gap-2 ">
          {pila2.elementos.map((item, index) => (
            <div
              key={index}
              className="bg-blue-500 text-white font-bold px-6 py-2 rounded-md"
            >
        ...
        {/* <div className="grid grid-cols-3 border  gap-8">
          <button onClick={popPila} className="bg-green-500 rounded-md h-fit w-fit px-4 py-2">
            Pop
          </button>
...
function ColasOne() {
  ...
  const [pila2, setPila2] = useState(new Pila()); // Estado para la segunda pila, inicializado como una nueva instancia de la clase Pila
  ...
  // Función para desapilar un elemento de la pila y apilarlo en la segunda pila
  const popPila = () => {
    if (!pila.estaVacia()) { // Verifica si la pila no está vacía
      const elemento = pila.desapilar(); // Desapila un elemento de la pila
      const nuevaPila2 = new Pila([...pila2.elementos, elemento]); // Crea una nueva pila con el elemento desapilado agregado
      setPila2(nuevaPila2); // Actualiza el estado de la segunda pila
    }
  };
}
